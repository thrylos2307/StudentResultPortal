...
router.post('/update_pass', authenticated.checkLoggedIn, authenticated.stage2, async (req, res) => {
  ...
  if (oPass.length < 8 || nPass.length < 8) {
    // return res.json({
    //   "reson of error" : "format not correct",
    //   "length of old pass" : oPass.length,
    //   "length of new pass" : nPass.length
    // });
    return res.redirect('/');
  }
  ...
  if (nPass !== cnPass) {
    ...
    // return res.json({
    //   "reson of error" : "Password and confirm password does not match"
    // });
  ...
  if (!pMatch) {
    ...
    // return res.json({
    //   "reson of error" : "Incorrect password"
    // });
    return res.redirect('/users/change_pass');
  }
  ...
  // return res.json({
  //   "success" : "Password updates successfully"
  // });
  return res.redirect('/');
});
...
router.post('/forget_password', authenticated.checkLoggedIn, async (req, res) => {
  ...
  await f_user.findOne({ userId: req.user._id }, (err, data) => {
    if (err) {
      req.flash('error', 'Please enter correct id');
